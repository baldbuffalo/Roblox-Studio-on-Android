...
public class MainActivity extends AppCompatActivity {
    ...
    private void runWindowsExecutable(String displayEnv) {
        try {
            ...
            // Exec directly rather than wrapping in "/system/bin/sh -c ...".
            // On some Android builds, spawning /system/bin/sh transitions
            // the child into a separate, more restricted SELinux domain
            // that lacks exec permission on apk_data_file (nativeLibraryDir
            // contents), even though the app's own process can read/exec
            // them fine. Passing the binary as argv[0] directly keeps the
            // child in the app's own domain.
            ...
            // glibcDir first: box64 itself is a real glibc-linked binary
